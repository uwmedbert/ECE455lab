...
// ---------- GPU Launcher ----------
int reduce_cuda_shared(const std::vector<int>& h_in, float& avg_time_ms) {
    int *d_in, *d_out;
    int h_out = 0;
...
// ---------- Main ----------
int main() {
    std::cout << "Running Shared-Memory Reduction (N = " << N << ")...\n";
    ...
    int gpu_result = reduce_cuda_shared(h_in, avg_gpu_time);
